...
OPAMPSUPERVISOR_VERSION?=v0.153.0
SKIP_COMPILE=false
ARCH?=amd64
...
opampsupervisor:
	...
	OPAMPSUPERVISOR_BUILD_DIR="$$(mktemp -d)"; \
	trap 'rm -rf "$$OPAMPSUPERVISOR_BUILD_DIR"' EXIT; \
	cp -R "$$(go env GOMODCACHE)/github.com/open-telemetry/opentelemetry-collector-contrib/cmd/opampsupervisor@$(OPAMPSUPERVISOR_VERSION)/." "$$OPAMPSUPERVISOR_BUILD_DIR"; \
	chmod -R u+w "$$OPAMPSUPERVISOR_BUILD_DIR"; \
	grep -v '=> \.\./\.\./' "$$OPAMPSUPERVISOR_BUILD_DIR/go.mod" > "$$OPAMPSUPERVISOR_BUILD_DIR/go.mod.tmp"; \
	mv "$$OPAMPSUPERVISOR_BUILD_DIR/go.mod.tmp" "$$OPAMPSUPERVISOR_BUILD_DIR/go.mod"; \
	cd "$$OPAMPSUPERVISOR_BUILD_DIR" && GO111MODULE=on CGO_ENABLED=$(CGO_ENABLED) GOOS=$(GOOS) GOARCH=$(GOARCH) go build -mod=mod -trimpath -o "$(CURDIR)/bin/opampsupervisor_$(GOOS)_$(GOARCH)$(EXTENSION)" .
ifeq ($(OS), Windows_NT)
